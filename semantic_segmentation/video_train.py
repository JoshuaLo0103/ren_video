...
class Trainer():
    def __init__(self, config):
        ...
        self.hidden_dim = config['ren']['architecture']['hidden_dim']
        ...
        self.decoder_checkpoint = os.path.join(self.exp_dir, 'no temperature decoder.pth')
        self.load_ren()
        self.load_decoder()
    ...
    def step(self, batch):
        ...
        with autocast(dtype=torch.bfloat16):
            ...
            with torch.no_grad():
                ...
                prompts = [self.grid_points for _ in range(B*T)]
                region_tokens = self.region_encoder(feature_maps, prompts)['pred_tokens']
                #region_tokens_new = self.mlp(region_tokens)
                region_tokens = region_tokens.view(B, T, self.grid_size, self.grid_size, -1)
            
            x = region_tokens.permute(0, 1, 4, 2, 3).contiguous()
            x = x.view(B*T, self.hidden_dim, self.grid_size, self.grid_size)   
            ...
            for batch_idx, batch in enumerate(pbar):
                ...
                if (iter_count + 1) % self.logging_steps == 0:
                    ...
                    if val_loss < self.best_val_loss:
                        ...
                        self.save_decoder(epoch, iter_count, self.best_val_loss)
                        print(f" New best val_loss={self.best_val_loss:.6f}")
