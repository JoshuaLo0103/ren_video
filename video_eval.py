        ...
        with torch.no_grad():
            with autocast(dtype=torch.bfloat16):
                ...
                if aggregate_tokens:
                    ...
                    # attention not use in grouping match shape
                    ...
                    # build frame-aware grid points (t, y, x)
                    grid_points_video = []
                    gp = self.grid_points.to(pred.device)  
                    for b_idx in range(B):
                        pts = []
                        for t_idx in range(T):
                            tp = torch.full((N0, 1), t_idx, device=pred.device, dtype=gp.dtype) 
                            pts.append(torch.cat([tp, gp], dim=1))
                        grid_points_video.append(torch.cat(pts, dim=0))

                    # aggregate tokens
                    agg = self.token_aggregator(pred, proj, attn, grid_points_video)
                    region_tokens = agg["aggregated_pred_tokens"]  
                    grouped_points = agg["all_grouped_points"]     
                    ...
                    padded_region_tokens = torch.stack(padded_region_tokens, dim=0)           
                    padded_region_tokens = padded_region_tokens.view(B, self.grid_size, self.grid_size, D)

                    # pass it to the decoder
                    logits_grid = self.decoder(padded_region_tokens.permute(0, 3, 1, 2))        
                    K = logits_grid.shape[1]
                    logits_groups = logits_grid.permute(0, 2, 3, 1).reshape(B, N0, K)         
...
if __name__ == '__main__':
    ...
    evaluator.run(split="val", aggregate_tokens=True, num_passes=5, max_batches=None, start_epoch=0)
